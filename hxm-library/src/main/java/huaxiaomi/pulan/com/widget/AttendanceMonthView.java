    ...
    protected void onDrawText(Canvas canvas, Calendar calendar, int x, int y, boolean hasScheme, boolean isSelected) {
        ...
        if (isSelected && (isClickDate || calendar.isCurrentDay())) {
            ...
            if(!TextUtils.isEmpty(scheme)){
                canvas.drawText(scheme, cx, mTextBaseLine + y + mItemHeight / 10 + 5, mSelectedLunarTextPaint);
            }
        } else if (hasScheme) {

            rawColor = mSchemeTextPaint.getColor();
            if(calendar.isWeekend()){
                mSchemeTextPaint.setColor(getResources().getColor(R.color.hxm_red));
            }
            canvas.drawText(String.valueOf(calendar.getDay()), cx, mTextBaseLine + top,
                    calendar.isCurrentDay() ? mCurDayTextPaint :
                            calendar.isCurrentMonth() ? mSchemeTextPaint : mOtherMonthTextPaint);
            mSchemeTextPaint.setColor(rawColor);

            if(!TextUtils.isEmpty(scheme)){
                rawColor = mSchemeLunarTextPaint.getColor();
                mSchemeLunarTextPaint.setColor(calendar.getSchemeColor());
                canvas.drawText(scheme, cx, mTextBaseLine + y + mItemHeight / 10 + 5, mSchemeLunarTextPaint);
                mSchemeLunarTextPaint.setColor(rawColor);
            }
        } else {

            rawColor = mCurMonthTextPaint.getColor();
            if(calendar.isWeekend()){
                mCurMonthTextPaint.setColor(getResources().getColor(R.color.hxm_red));
            }
            canvas.drawText(String.valueOf(calendar.getDay()), cx, mTextBaseLine + top,
                    calendar.isCurrentDay() ? mCurDayTextPaint :
                            calendar.isCurrentMonth() ? mCurMonthTextPaint : mOtherMonthTextPaint);
            mCurMonthTextPaint.setColor(rawColor);
